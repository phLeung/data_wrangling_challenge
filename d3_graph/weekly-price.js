...
var svg = d3.select("#weeks").append("svg")
   .attr("width", width + margin.left + margin.right)
   .attr("height", height + margin.top + margin.bottom)
    .append("g")
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
var parseDate = d3.timeParse("%Y-%m-%d");
d3.csv("https://raw.githubusercontent.com/phLeung/data_wrangling_challenge/master/data/weekly_prices.csv")
...
.get(function(error,data)
{
   ...
   //establish line
   var weeklyPriceLine = d3.line()
   .x(function(d){ return x(d.date); })
   .y(function(d) { return y(d.price); });
   svg.append("g").attr("transform", "translate(0," + height + ")").call(d3.axisBottom(x));
   svg.append("g").call(d3.axisLeft(y));
   //add line
   svg.append("path").datum(data).attr("class","lines").attr("d",weeklyPriceLine);
   //label for x axis
   svg.append("text")
   .attr("transform",
